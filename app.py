...
def supplier_add():
    if request.method == 'POST':
         ...
         cn.execute(f"insert into supplier(supplier_name,supplier_address,supplier_email)values('{supplier_name}','{supplier_address}','{supplier_email}')")
         conn.commit()
         print('data has been inserted')
         return jsonify({'message':'sucessful'})
    else:
